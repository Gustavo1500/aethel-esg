import numpy as np
import pandas as pd
from actuarial_esg import MarketSimulator, MarketCalibrator, SimulationResults


def generate_mock_historical_data(months: int = 120):
    np.random.seed(101)
    historical_ipca = 0.045 + np.random.normal(0, 0.005, months)
    historical_cdi = historical_ipca + 0.04 + np.random.normal(0, 0.008, months)
    historical_cdi = np.maximum(0.01, historical_cdi)
    ...
    return historical_ipca, historical_cdi, historical_equity, historical_curve


def run_full_pipeline():
    print("==================================================")
    print(" Actuarial ESG - Complete Pipeline Execution Demo ")
    print("==================================================")

    # Step 1: Generate/Import historical market data
    months_of_history = 120
    hist_ipca, hist_cdi, hist_eq, hist_curve = generate_mock_historical_data(months=months_of_history)
    ...
    calibrated_config = calibrator.fit(
        historical_ipca=hist_ipca,
        historical_cdi=hist_cdi,
    ...
    # Step 5: Export a single path for a FIRE simulator run-through (retains to_pandas support)
    ...
    # Using the clean query API instead of manual slice manipulation
