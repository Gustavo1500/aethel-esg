...
def run_visualization_suite():
    print("==================================================")
    print("      Actuarial ESG - Interactive Dashboard       ")
    print("==================================================")

    # 1. Setup a standard 30-year configuration with 500 scenarios
    config = SimulatorConfig(
        duration_years=30,
        num_scenarios=500,
        seed=42,
        initial_cdi=0.105,   # Starting short rate at 10.5%
        initial_ipca=0.045   # Starting inflation at 4.5%
    )
    ...
    fig_paths = results.plot_scenario_paths(metric="cdi", num_paths=15, title="Sample Short Rate (CDI) Trajectories")
    ...
    dashboard = make_subplots(
        ...
        subplot_titles=(
            ...
            "Sample Short Rate (CDI) Trajectories",
    ...
    # Map Traces from independent charts into target Subplot Quadrants
    # Quadrant (1, 1): Fan Chart
    for trace in fig_fan.data:
        dashboard.add_trace(trace, row=1, col=1)

    # Quadrant (1, 2): Scenario Spaghetti Paths
    for trace in fig_paths.data:
        dashboard.add_trace(trace, row=1, col=2)

    # Quadrant (2, 1): Term Structure Evolution
    for trace in fig_curves.data:
        dashboard.add_trace(trace, row=2, col=1)

    # Quadrant (2, 2): Ending Equity Distribution Histogram
    for trace in fig_dist.data:
        dashboard.add_trace(trace, row=2, col=2)

    # 5. Extract statistics to re-apply the vertical reference lines in the Histogram Quadrant (2, 2)
    ...
    dashboard.update_yaxes(title_text="Annualized CDI Rate", row=1, col=2)
    ...
    dashboard.update_layout(
        title_text="Actuarial Economic Scenario Generator (ESG) Dashboard",
        height=950,
        width=1450,
    ...
    # 7. Open single unified web browser window
    dashboard.show()
