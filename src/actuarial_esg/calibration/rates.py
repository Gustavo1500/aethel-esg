...
class RatesCalibrator:
    ...
    def calibrate_short_rate_series(historical_rates: np.ndarray, dt: float = 1.0 / 12.0) -> dict:
        ...
        # Discretized form: (r_{t+1} - r_t) / sqrt(r_t) = theta * mu * dt / sqrt(r_t) - theta * dt * sqrt(r_t) + error
        r_t = r[:-1]
        r_next = r[1:]
        ...
        if theta <= 0.0:
            theta = 0.25  # Fallback target
        mu = np.maximum(0.01, theta_mu / theta)

        # Estimate residuals to recover historical diffusion volatility
    ...
    def fit_yield_curve_to_target(
        ...
        # Objective: minimize Mean Squared Error (MSE) between historical targets and model
        def objective(params):
            theta, mu, sigma = params
            if theta <= 0.01 or mu <= 0.001 or sigma <= 0.001:
                return 1e10
            
            # Analytical CIR formula
        ...
        if not res.success:
            # Fallback to realistic expectations
